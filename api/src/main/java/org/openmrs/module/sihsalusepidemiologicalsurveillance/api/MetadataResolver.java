...
public class MetadataResolver {
	...
	public Metadata get() {
		try {
			String json = clinical.property(SurveillanceConstants.MODULE_ID + ".metadata");
			if (json == null || json.trim().isEmpty())
				throw new IllegalArgumentException();
			Metadata m = new ObjectMapper().readValue(json, Metadata.class);
			validate(m);
			return m;
		}
		catch (Exception ex) {
	...
	public void validate(Metadata m) {
		...
		datatypes.put("pregnancy", "Boolean");
		for (String key : Arrays.asList("sourceEncounter", "laboratoryResult", "ethnicity"))
		...
		for (Metadata.Disease disease : m.diseases) {
			...
			allowedAnswer(m.questions.get("event"), event.getConcept().getUuid());
			...
			for (Metadata.DiagnosisMapping mapping : disease.diagnoses) {
				...
				icd10(requiredConcept(mapping.diagnosisConceptUuid), m);
			}
